...
function username_exists($email){
	$email = sanitize($email);
	return (mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `users` WHERE `username` = '$email'"), 0)==1) ? true : false;
...
function getName($user_id){
	$user_id = sanitize($user_id);
	$result = mysql_query("SELECT `fname`,`lname` FROM `users` WHERE `user_id` = '$user_id'");
	return($result);
}
?>